...
//   // ADESSO QUI: check start==goal sempre valutato
...
// Integration with additional checks + Step 5 (Cost Map)
#include "simple_planner/simple_planner.hpp"
#include "simple_planner/map_utils.hpp"
...
void SimplePlanner::timerCallback()
{
  ...
  if (!isInBounds(start_cell_) || !isInBounds(goal_cell_)) {
    RCLCPP_WARN(this->get_logger(),
                "Start or goal out of map bounds. Skipping planning.");
    publishPath({});
    return;
  }
